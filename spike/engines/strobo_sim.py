...
tooth, delta = k / DELTA_t = k * f_lf -- the carrier (k=0) and the FIRST SIDEBANDS
(k=+-1, at +- the motional frequency). The OC k-vector gradient (Lamb-Dicke eta) gives
the pulse a motional displacement D(i eta), so each pulse couples |up,n> to |down,m>
with the carrier (m=n) and sideband (m=n+-1) Lamb-Dicke strengths.

We simulate the exact stroboscopic evolution on a small spin x Fock Hilbert space
(start |up, 0>):
...
The red sideband is suppressed for a cold |0> ion (it cannot remove a phonon), exactly
as in the sideband thermometry -- so the simulated comb is red/blue ASYMMETRIC for the
ground state. Pure Python (built-in complex), no numpy: a few small (2F x 2F) matrices.
"""
